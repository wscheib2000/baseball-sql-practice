...
FROM
  (
    ...
    WHERE tmp.GS_prop > 0.75 AND tmp.year >= 1990 AND tmp.innings_pitched >= 50
  ) AS tbl
LEFT JOIN
(
...
mean = mean(data$weighted_TTO_above_yearly_avg)
sd = sd(data$weighted_TTO_above_yearly_avg)
binwidth = (max(data$weighted_TTO_above_yearly_avg) - min(data$weighted_TTO_above_yearly_avg)) / 20
n = nrow(data)
...
# Calculate parameters
mean = mean(data$wght_TTO_abv_avg_adj)
sd = sd(data$wght_TTO_abv_avg_adj)
binwidth = (max(data$wght_TTO_abv_avg_adj) - min(data$wght_TTO_abv_avg_adj)) / 20
n = nrow(data)
